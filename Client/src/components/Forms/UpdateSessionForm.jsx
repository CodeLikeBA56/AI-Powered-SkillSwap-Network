...
import { formatDateForInput } from '../../services/Date.js';
import { useAlertContext } from '../../context/AlertContext';
import { useUserSessions } from '../../context/UserSessionsProvider.jsx';

const UpdateSessionForm = ({ session, closeUpdateSessionForm }) => {
    const { showAlert } = useAlertContext();
    const { updateSession } = useUserSessions();
    ...
    useEffect(() => {
        if (session) {
            ...
            setHashtags(session.hashtags.join(', '));
        }
    }, [session]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axiosInstance.put(`api/session/${session._id}`, sessionData);
            const { type, message, updatedSession } = response.data;
    ...
    return (
        <div className="form-container">
            ...
            <h2 className='form-title'>Update Session</h2>
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit" className="btn session-btn">Update Session</button>
            </form>
        </div>
    );
};
