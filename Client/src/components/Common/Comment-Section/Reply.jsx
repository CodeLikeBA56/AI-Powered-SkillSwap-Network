...
const Reply = ({ post, comment, reply, getFormattedTime, onUpdatedReply, commentSectionType }) => {
    ...
    useEffect(() => {
        if (userInfo) {
            setLikedByUser(reply.likes.some(id => id === userInfo?._id));
        }
    }, [userInfo])
    ...
    const handleReplyLike = async () => {
        ...
        try {
            const response = await axiosInstance.put(`api/${commentSectionType}/${post}/comment/${comment}/like-reply/${reply._id}`);
            ...
            console.error("Error toggling like:", error.response?.data?.message || "Something went wrong");
        }
      };
  ...
  return (
    <div className='reply-container'>
        <img
            src={`${import.meta.env.VITE_SERVER_SIDE_API_URL}api/get-media/user-media/${reply.user.profilePicture}` || 'https://via.placeholder.com/150'}
            alt="Icon" 
        ...
        <div className='reply-user-info'>
            ...
            <span className='reply-date'>{getFormattedTime(reply.createdAt)}</span>
        </div>

        <button className='comment-like-btn' onClick={() => handleReplyLike()} type='button' style={{color: true === likedByUser? "var(--main-color)" : "var(--text-color)"}}>
            {
                true === likedByUser?
